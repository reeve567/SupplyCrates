	...
	public void run() {
		...
		if (time == 0) {
			...
			if (broadcast) {
				...
				Bukkit.getServer().broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&8&l[&4&lSPC&8&l] &cSupplyCrates have been dropped in the " +
						"Warzone go get them quick!"));
				for (int i = 0; i < 100; i++) {
				...
				Bukkit.getServer().broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&8&l[&4&lSPC&8&l] &cSupplyCrates have been removed."));
				broadcast = true;
				for (int i = 0; i < 100; i++) {
